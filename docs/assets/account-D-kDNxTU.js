import"./modulepreload-polyfill-B5Qt9EMX.js";if(window.top!==window.self){try{window.top.location.replace(window.location.href)}catch{document.documentElement.innerHTML=""}throw new Error("framed")}const y=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:3030":"https://autoyield-api.fly.dev",g=new Set(["autoyield.org","www.autoyield.org","localhost","127.0.0.1"]);if(!g.has(location.hostname))throw document.body.innerHTML=`
    ...
    </div>`,new Error("blocked");const p=location.hostname==="localhost"||location.hostname==="127.0.0.1",w=p?new URLSearchParams(location.search).get("api"):null,d=(w??y).replace(/\/$/,""),o=document.getElementById("signin"),c=document.getElementById("status"),m=document.getElementById("result");function a(t,n=""){if(t==="hidden"){c.style.display="none";return}c.style.display="block",c.className=`status${t==="info"?"":" "+t}`,c.textContent=n}function f(){var n;const t=((n=window.phantom)==null?void 0:n.solana)??window.solana;return t!=null&&t.isPhantom?t:null}function b(t){let n="";for(let i=0;i<t.length;i+=32768)n+=String.fromCharCode.apply(null,Array.from(t.slice(i,i+32768)));return btoa(n)}o.addEventListener("click",async()=>{a("hidden"),m.style.display="none";const t=f();if(!t){a("warn",`Phantom not detected.
